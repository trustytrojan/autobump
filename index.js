import { Client, Message, Modal, TextChannel } from 'discord.js-selfbot-v13';
 ...
 */
const bump = async () => {
	// sometimes the interactions fails: catch and return void in this case
	const msg = await channel.sendSlash(DISBOARD_ID, 'bump').catch(() => {});

	// if this is a modal, disboard changed quite a lot...
	assert(!(msg instanceof Modal));

	if (!msg || msg.content.startsWith('The DISBOARD API')) {
		log(`DISBOARD (or its API) is down... waiting a minute to try again`);
